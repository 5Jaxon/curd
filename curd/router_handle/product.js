...
//通过部门对产品搜索
exports.searchProductForOutDe = (req, res) => {
	const sql = 'select * from outproduct,users where outproduct.product_out_apply_person=users.name and users.department   = ?'
	db.query(sql, req.body.product_out_De, (err, result) => {
		if (err) return res.cc(err)
		res.send(result)
	})
}
